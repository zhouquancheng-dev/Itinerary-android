...
import com.example.ui.theme.helloPathColors
import kotlin.math.floor

@Composable
@Preview
fun HelloPathAnimation(modifier: Modifier = Modifier) {
    val path = remember {
    ...
    Box(modifier = modifier) {
        Canvas(
            modifier = Modifier
                .padding(start = 84.dp, end = 32.dp, top = 32.dp, bottom = 32.dp)
            ...
            lines.forEach { line ->
                if (line.startFraction * totalLength < currentLength) {
                    val startColor = interpolateColors(line.startFraction, helloPathColors)
                    val endColor = interpolateColors(line.endFraction, helloPathColors)
                    drawLine(
